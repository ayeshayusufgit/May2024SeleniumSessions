...
public class ElementUtil7 {
	...
	public ElementUtil7(WebDriver driver) {
		this.driver=driver;
	...
	public  boolean waitForURLContains(String parURL, int timeOut) {
		...
		try {
			return wait.until(ExpectedConditions.urlContains(parURL));// return true if the part title is contained in the title
		} catch (TimeoutException ex) {
			System.out.println("Part URL is not matched");
			return flag;// returns false if the part URL is not present in the URL
		}
	}
